...
%% Área estudada
ag1 = LongToRA(lg1); % Ascens~
ao inicial da área a ser estudada
dg1 = lt1; % Declinaç~
ao inicial da área a ser estudada
ag2 = LongToRA(lg2); % Ascens~
ao final da área a ser estudada
dg2 = lt2; % Declinaç~
ao final da área a ser estudada
da = nd_space;
dd = nd_space;
...
dg = dg1+dd/2:dd:dg2-dd/2; % Declinaçoes dos centros das discretizaç~
oes em graus
agr = ag*pi/180; % Ascensoes dos centros das discrtizaç~
oes em radianos
dgr = dg*pi/180; % Declinaçoes dos centros das discrtizaç~
oes em radianos
nag = length(ag);
ndg = length(dg);
Area_Total = nag*ndg;
...
Eg = vpa(Er*180/pi); %E (Anomalia Exc^
entrica) em graus
...
% Ascens~
ao Reta Geoc^
entrica do satélite em graus
51
ag0r = vpa(ag0*pi/180); % Ascens~
ao Reta Geoc^
entrica do satélite em radianos
dg0r = (sin(ur)/abs(sin(ur)))*acos(cos(ur)/cos(dalphar));
% Declinaç~
ao Geoc^
entrica do satélite em radianos
dg0 = vpa(dgr*180/pi); % Declinaç~
ao Geoc^
entrica em graus
thetar = theta*pi/180;
...
% Metade do a
^ ngulo de vis~
ao da Área de cobertura do centro da Terra
for k = 1:nag
for j = 1:ndg
