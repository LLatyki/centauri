...
function propagation(element_orb)
    ## Entradas
...
function sideral_time(date::Dates.DateTime)
    #REVISAR FUNÇAO

    y = Dates.year(date)
    mon = Dates.month(date)
    d = Dates.day(date)
    h = Dates.hour(date) + 3
...
function fix(float::Float64)::Int
